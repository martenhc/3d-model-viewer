...
export class VectorControlElement extends LitElement {
  ...
  @property({type: Function}) onValueChanged!: (vectorUpdate: VectorUpdate) => void;
  ...
  protected firstUpdated(): void {
    ...
    function _onInput(this: GlobalEventHandlers) {
      ...
      const axisIndex = parseInt((this as HTMLInputElement).getAttribute('data-axis-index') || '0');
      ...
      that.onValueChanged({hotspotIndex: that.hotspotIndex, value: parseInt(passedValue) / 100, axisIndex});
    }
  ...
  render() {
    return ['x', 'y', 'z'].map((axis, index) =>
      html`${axis}:<div>
        <input data-axis-index=${index} class="slidebar" type="range" min="-100" max="100" value="0" />
        <input data-axis-index=${index} class="number" type="number" value="0" min="-100" max="100" />
      </div>`)
  }
}
